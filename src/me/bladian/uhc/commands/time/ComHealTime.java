 ...
 */
public class ComHealTime implements CommandExecutor
{
    ...
    {
        if(command.getName().equalsIgnoreCase("healTime"))
        {
            if(commandSender.hasPermission("rank.host"))
            {
                if(strings.length == 0)
                {
                    commandSender.sendMessage(reference.ERROR + "§c/healTime <minute>");
                    return true;
                }
                ...
                int newBorder = Integer.parseInt(strings[0]);
                gameManager.setHealTime(newBorder);
                Bukkit.broadcastMessage(reference.GAME + "§7Heal time§f is now set to§7 " + strings[0]);
            }
        }
        return false;
    }
